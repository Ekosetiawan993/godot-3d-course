...
func defend(forward: bool) -> void:
	...
	tween.tween_method(_defend_change, 1 - float(forward), float(forward), 0.25)
...
func cast_spell() -> void:
	if not attacking:
		# change the extra animation to Spellcast_Shoot
...
func _on_blink_timer_timeout() -> void:
	...
	change_face("default")
	$BlinkTimer.wait_time = rng.randf_range(1.2, 2.5)
